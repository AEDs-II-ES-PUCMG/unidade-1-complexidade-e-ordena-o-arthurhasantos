...
public class Pedido implements Comparable<Pedido>{
	...
	public double valorFinal() {
		...
		for (int i = 0; i < quantItens; i++) {
			BigDecimal sub = BigDecimal.valueOf(itens[i].getPrecoVenda())
					.multiply(BigDecimal.valueOf(itens[i].getQuantidade()));
			total = total.add(sub);
		}
		if (formaDePagamento == 1) {
			BigDecimal fator = BigDecimal.ONE.subtract(BigDecimal.valueOf(DESCONTO_PG_A_VISTA));
			total = total.multiply(fator);
		}
    ...
    public double valorCatalogoAtual() {
        ...
        for (int i = 0; i < quantItens; i++) {
            ...
            BigDecimal sub = BigDecimal.valueOf(precoAtual)
                    .multiply(BigDecimal.valueOf(itens[i].getQuantidade()));
            soma = soma.add(sub);
        }
        return soma.setScale(2, RoundingMode.HALF_UP).doubleValue();
    }
    ...
    public double indiceEconomia() {
        BigDecimal cat = BigDecimal.valueOf(valorCatalogoAtual());
        BigDecimal fin = BigDecimal.valueOf(valorFinal());
        return cat.subtract(fin).setScale(2, RoundingMode.HALF_UP).doubleValue();
    }
